...
{   //画像をPDFにするところ  
    ...
    public void ConvertMultipageTifToBmpKeepDpi(string tif, string outputDir)
    {
        using var img = Image.FromFile(tif);
        ...
        for (int i = 0; i < frameCount; i++)
        {
            ...
            // 出力ファイル名（page_0.bmp など）
            string outPath = Path.Combine(outputDir, $"page_{i}.bmp");
